...
The length composititions the Onboard Index in the Southern model showed a large change between the early sampling period prior to 1999 and the later period starting in 2003. The patterns were relatively stable within each period, suggesting that the change was likely due to changes in fishing patterns or sampling, rather than changes in the underlying stock. Therefore, separate selectivity parameters were estimated for the two periods. Although the changes in the data were less dramatic, separate selectivity patterns werealso estimated for the Recreational Fishery (lengths from MRFSS and RecFIN) estimated with the later period starting in 1993 after the gap in sampling that started in 1990. In both cases, the change in selectivity was also paired with an estimated change in catchability for the associated indices of abundance.
...
We also evaluated different assumptions pertaining to maturity ogives, modeling these parameters from the literature:
...
The Southern jitter analysis post-tuning was run 100 times, and resulted in 75 models that returned to the base case.  No model resulted in a lower likelihood than the base model.
...
## Southern Example Model Results

The results offered here are for a version of the Southern model that was thought to be the most robust among sensitivites, and is not a "Base Case", as the model was deemed too sensitive for management.
...
### Southern Example Model Selectivities, Indices and Discards
...
### Southern Example Model Lengths
...
The Small Fish Study lengths are not fit badly (Figures \ref{fig:mod2_17_comp_lenfit_flt6mkt2} and \ref{fig:mod2_20_comp_lenfit_data_weighting_TA1.8_Small Fish Study}), and it is perhaps a shame that there are so few years to this timeseries.
...
Figure \ref{fig:mod2_5_comp_agefit_flt6mkt2} shows the fit to the Recreational Fishery samples, which is poor in all four years.  The mean age in this data is shown in Figure \ref{fig:mod2_8_comp_agefit_data_weighting_TA1.8_Small Fish Study}, at 10 years.

The Hook-and-Line Survey age fit is shown in Figure \ref{fig:mod2_1_comp_agefit_flt4mkt0}.  The Francis tuning method could not be applied in this case as it depends on the fit to multiple years of data.
...
### Southern Example Model Parameters
...
### Southern Example Model Uncertainty and Sensitivity Analyses

The Example Southern model was investigated in these 16 analyses:
...
### Southern Example Model Likelihood Profiles
...
### Southern Example Model Retrospective Analysis
...
### Southern Example Model Reference Points
