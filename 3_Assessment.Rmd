...
\hl{add something about DB-SRA or data poor to get component OFLs for Southern stock}
...
The Southern jitter analysis post-tuning was run 100 times, and resulted in 43 of a hundred models that returned to the base case.  No model resulted in a lower likelihood than the base model.

The Northern model \hl{report jittering when it's been done}.
...
For the Base model, the parameter estimates are given in Table \ref{tab:Model1_params}.  Status for all of the estimated parameters is good although the parameter for peak selectivity of the Triennial survey as estimated close the 55 cm upper bound with a value of `r round(mod1$estimated_non_dev_parameters["SizeSel_P1_Triennial(5)", "Value"],2)`.

### Northern Model Uncertainty and Sensitivity Analyses
\begin{description}
\item[M prior] In the Northern model, natural mortality was estimated at values very close to the median of the chosen prior distributions in spite of the prior inadvertently having been turned off. This sensitivity explores the impact of including the prior. 
...
Results of these investigations can be seen in Figure \ref{fig:sens.N.spawnbio} and Table \ref{tab:Sensitivity_model1}.

\hl{Text discussing sensitivity results to be added.}
...
\begin{description}
\item[Drop Biological Datasets]The data from each source in turn was dropped from the model.  This resulted in improved Total Likelihoods, but implausible results.  Three models (those without the Small Fish, without the Onboard lengths and without the Commercial data) had timeseries with spawning biomass higher than virgin spawning biomass in some years.  Removal of the Hook and Line data or the Onboard data results in models with dramatically increased uncertainty.
The model without the Hook and Line data and the model lacking the Small Fish both estimate much higher fishing mortality than the base model.

\item[Drop Indices]Each index in turn was dropped from the model.  This made very little difference in Total Likelihoods, but except for the model lacking the Juvenile index, estimated virgin spawning biomass 5 orders of magnitude smaller than the base model, however all models ended up with similar timeseries and similar relative depletion.

\item[Changes to M]Three sensitivities to M were run:  we let the model estimate M, we fixed M at the northern values, and we fixed M at a value that Hamel[-@Hamel2015] estimated for a maximum age of 64, the value reported in [-@Love2011].  The model is very sensitive to changes in M.
...
Results of these investigations can be seen in Figures \ref{xxx}, and Table \ref{yyy}.
