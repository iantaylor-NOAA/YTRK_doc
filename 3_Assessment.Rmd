...
\emph{Recreational}: The recreational fleet includes data from sport fishery off the California coast south of Cape Mendocino.  The recreational lengths are unsexed.  The index is in fish per angler-hour.  Further information about how the index was worked up is in Appendix \ref{xxx}.
...
The estimated parameters are described in more detail below and a full list of all estimated and fixed parameters is provided in Table \ref{tab:Model1_params} (Northern model) and Table \ref{tab:Model2_params} (Southern model).
...
\hl{Stacey suggests "Description is very sparse in this section. Would be helpful to reviewers to have more information on what was done and what models fits were, rather than just saying they were obviously poor."}
...
The small fish survey lengths are not fit badly (Figures \ref{fig:mod2_17_comp_lenfit_flt5mkt2} and \ref{fig:mod2_20_comp_lenfit_data_weighting_TA1.8_Recreational Study}), and it is perhaps a shame that there are so few years to this timeseries.
...
###Results of Jitter Runs
We jittered the model 100 times to ensure that the best parameter values are those in the final base-case.  The jitter runs returned to the base case (likelihood 774.696) 43 times, and no run resulted in a lower likelihood.
