...
The data associated with the commercial fleet includes age- and length-composition data from PacFIN and CalCOM, historical catch timeseries from CDFW, ODFW and WDFW. Observations of discards from the Pikitch research study provide lengths and discard rates; the West Coast Groundfish Observing Program (**WCGOP**), which documents discarding in the commercial fishery (providing discard lengths), as well as discard rates calculated from WCGOP data.  Sex was available for the comps in the retained catch, which is by-sex in the model, but was not available for the discards, so they are undifferentiated by sex.
...
Northwest Fisheries Science Center's **NWFSCcombo** survey each provide age- and length-compositions, as well as an index of abundance.
...
Selectivities in the Northern model (Figure \ref{fig:selex_model1}) shows the difference between the recreational fisheries and the commercial fishery and survey sampling.  All of the fish are fully selected by 50 cm, but the recreational fish are fully selected at 30 cm.

Retention by length (Figure \ref{fig: retention_model1}) varies over time between 40\% and 100\%, with no clear pattern of interannual variation, except for the trawl-rationalization era 2011-present.

Discarding in the commercial fleet (Figure \ref{fig:r4ss_discard_model1}) is fit only by putting blocks on retention in the Northern model.  Discards were very low except during the 1990s and 2000s, until the trawl-rationalization program implementation.
...
Commercial length comps are very well fit (Figures \ref{fig:xxx} and \ref{fig:mod1_5_comp_lenfit_data_weighting_xxx}).  Commercial discards are noiser and not well fit (Figure \ref{fig:mod1_6_comp_lenfxxx}).  The panel describing the combined fits and data weighting for the commercial fishery is duplicted, need to remove redundant figure.

Lengths in the early period of the Hake Bycatch fishery are noisy (doubtless due to small sample sizes).  By 1992, the model is able to fit the data well (Figure \ref{fig:mod1_10_comp_xxx}).  Figure \ref{fig:mod1_13_comp_lenfit_data_weighting} shows that the fits in the early period have twice (at last) the uncertainty of the later period.
...
Figure \ref{fig:profile_M2.N} shows the likelihood profile for male natural mortality, MM, over a range of negative values that are the offset from female mortality (FM).  The index data are again at odds with the other data sources; all but the indices are minimized at a value of -0.15 \hl{more explanation here, this is not a natural number}.  The indices prefer both larger *and* smaller values of the offset.
...
Male natural mortality (MM) is profiled over a range from -0.4 to 0.  \hl{These values are offsets from female natural mortality}.  All roads lead to Rome in this figure (Figure \ref{fig:profile_M2.S}); since all data sources and the total are minimized at zero.  Likelihoods for recruitments and indices are flat over the range of MM; the other data sources show changes of 20 (lengths) and 80 (ages) likelihood values.
